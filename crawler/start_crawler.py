...
from crawler.base_crawler import CrawlerType0, CrawlerType1, CrawlerType2
...
class HindilyricsCrawler(CrawlerType0):
    ...
    def get_song_details(self, raw_html):
        singers = modify_artist_hl(
            findall(r'Singer\(s\).*?:(.*?)<br>', raw_html))
        music_by = modify_artist_hl(findall(r'Music By.*?:(.*?)<br>', raw_html))
        lyricists = modify_artist_hl(
            findall(r'Lyricist.*?:(.*?)<br>', raw_html))
